  ...
  if (theme === 'Ink') {
    if (face.startsWith('B')) {
      ...
      for (let i = 0; i < lineCount; i++) {
        ...
        g.appendChild(svgEl('line', { x1: cx - 20, y1: y, x2: cx + 20, y2: y, stroke: color, 'stroke-width': 2.5, 'stroke-linecap': 'round' }));
      }
      g.appendChild(svgEl('line', { x1: cx - 22, y1: cy - 22, x2: cx - 22, y2: cy + 22, stroke: color, 'stroke-width': 1.5 }));
    } else if (face.startsWith('C')) {
      const num = parseInt(face[1]);
      // Ink characters: simple block numerals
      const blockNums = ['Ⅰ', 'Ⅱ', 'Ⅲ', 'Ⅳ', 'Ⅴ', 'Ⅵ', 'Ⅶ', 'Ⅷ', 'Ⅸ'];
      g.appendChild(svgEl('text', { x: cx, y: cy + 18, 'text-anchor': 'middle', style: `font:bold 48px 'Courier New';fill:${color};pointer-events:none;` })).textContent = blockNums[num - 1];
      ...
      g.appendChild(svgEl('path', { d: shapes[face], stroke: color, 'stroke-width': 2.5, fill: 'none', 'stroke-linejoin': 'round' }));
    } else if (face.startsWith('D')) {
      const num = parseInt(face[1]);
      // Ink circles: grid of small dots
      ...
      const windDirs = { E: 'M 34 46 L 50 46 M 48 42 L 50 46 L 48 50', S: 'M 46 34 L 46 50 M 42 48 L 46 50 L 50 48', W: 'M 50 46 L 34 46 M 36 42 L 34 46 L 36 50', N: 'M 46 50 L 46 34 M 42 36 L 46 34 L 50 36' };
      g.appendChild(svgEl('path', { d: windDirs[face.substring(1)] || '', stroke: color, 'stroke-width': 2.5, fill: 'none', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }));
    } else if (face.startsWith('F')) {
      const num = parseInt(face[1]);
      // Ink flowers: concentric circles
      g.appendChild(svgEl('circle', { cx, cy, r: 4, fill: color }));
      g.appendChild(svgEl('circle', { cx, cy, r: 12, stroke: color, 'stroke-width': 2, fill: 'none' }));
      g.appendChild(svgEl('circle', { cx, cy, r: 22, stroke: color, 'stroke-width': 1.5, fill: 'none' }));
      g.appendChild(svgEl('text', { x: cx + 24, y: cy + 28, 'text-anchor': 'middle', style: `font:bold 14px Arial;fill:${color};pointer-events:none;` })).textContent = num;
      ...
      g.appendChild(svgEl('path', { d: patterns[num] || '', stroke: color, 'stroke-width': 2, fill: 'none', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }));
    }
  } else {
    // Classic theme: original designs
    if (face.startsWith('B')) {
      const num = parseInt(face[1]);
