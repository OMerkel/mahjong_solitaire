...
const updateHeaderBadge = (settings) => {
	...
	badge.setAttribute(
		"aria-label",
		`Layout ${layout}, theme ${theme}, free-tiles highlight ${free}`,
	);
};
...
const readSettings = () => ({
	layout:
		...
		"ClassicTurtle",
	showfreetiles:
		document.querySelector('input[name="showfreetiles"]:checked')?.value ??
		"On",
	showhints:
		document.querySelector('input[name="showhints"]:checked')?.value ?? "On",
	selectionmode:
		document.querySelector('input[name="selectionmode"]:checked')?.value ??
		"Flexible",
	tiletheme:
		document.querySelector('input[name="tiletheme"]:checked')?.value ??
		"Classic",
});
...
const startGameTimer = (board) => {
	...
	if (!board?.firstMoveTime || board.isWon || board.isBlocked) {
		return;
	}
	...
	timerInterval = setInterval(() => {
		...
		if (
			!currentBoard.firstMoveTime ||
			currentBoard.isWon ||
			currentBoard.isBlocked
		) {
			clearInterval(timerInterval);
			timerInterval = null;
			return;
		...
		const elapsedSeconds = Math.floor(
			(Date.now() - currentBoard.firstMoveTime) / 1000,
		);
...
const handleTileClick = (tileId) => {
	...
	const allowReselect =
		(state.settings.selectionMode ?? "Flexible") === "Flexible";
	...
	const action = state.selectableActions.find(
		(candidate) =>
...
store.subscribe((state) => {
	...
	const hintPair =
		...
		state.selectableActions.length > 0
			? [
					state.selectableActions[0].firstId,
					state.selectableActions[0].secondId,
				]
			: null;
...
const emptyScores = () => {
	const initLayout = () =>
		Object.fromEntries(LAYOUT_NAMES.map((layout) => [layout, null]));
	return {
		...
		if (next[period].period !== keys[period]) {
			next[period] = {
				...
				layouts: Object.fromEntries(
					LAYOUT_NAMES.map((layout) => [layout, null]),
				),
			};
		}
	}
...
const updateHighscoreView = (scores, currentLayout = "ClassicTurtle") => {
	...
	if (todayEl)
		todayEl.textContent = formatScoreWithLayout(
			scores.today.layouts,
			currentLayout,
		);
	if (weekEl)
		weekEl.textContent = formatScoreWithLayout(
			scores.week.layouts,
			currentLayout,
		);
	if (monthEl)
		monthEl.textContent = formatScoreWithLayout(
			scores.month.layouts,
			currentLayout,
		);
	...
	if (gameTodayEl)
		gameTodayEl.textContent = formatScoreWithLayout(
			scores.today.layouts,
			currentLayout,
		);
	if (gameWeekEl)
		gameWeekEl.textContent = formatScoreWithLayout(
			scores.week.layouts,
			currentLayout,
		);
	if (gameMonthEl)
		gameMonthEl.textContent = formatScoreWithLayout(
			scores.month.layouts,
			currentLayout,
		);
};
...
const recordCompletedGame = (board) => {
	if (!board?.firstMoveTime || !board.layout) return;
...
const resetHighscore = (period) => {
	...
	const next = refreshScorePeriods(loadScores());
	next[period].layouts = Object.fromEntries(
		LAYOUT_NAMES.map((layout) => [layout, null]),
	);
	saveScores(next);
	const currentLayout = store.getState().settings.layout ?? "ClassicTurtle";
...
const renderTileShowcase = (theme = "Classic") => {
	...
	const categories = {
		Bamboo: {
			faces: ["B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8", "B9"],
			copies: 4,
		},
		Characters: {
			faces: ["C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9"],
			copies: 4,
		},
		Disks: {
			faces: ["D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D9"],
			copies: 4,
		},
	...
	for (const [categoryName, categoryData] of Object.entries(categories)) {
		...
		const copyText =
			categoryData.copies === 1
				? "single tiles"
				: `${categoryData.copies} copies per tile`;
		...
		for (const face of categoryData.faces) {
			...
			const graphics = createTileGraphics(face, palette, theme);
...
const wireUI = () => {
	...
	document
		.querySelectorAll(
			'input[name="layout"], input[name="showfreetiles"], input[name="showhints"], input[name="selectionmode"], input[name="tiletheme"]',
		)
		.forEach((input) => {
