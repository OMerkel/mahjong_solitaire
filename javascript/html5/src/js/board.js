...
const points = (xStart, xEnd, yStart, yEnd, z) =>
	range(yStart, yEnd).flatMap((y) =>
		range(xStart, xEnd).map((x) => ({ x, y, z })),
	);
...
const createClassicTurtleLayout = () => {
	...
	const z1 = [...points(4, 9, 1, 6, 1)];

	// Layer z2: 4x4 grid (shifted right by 1)
	const z2 = [...points(5, 8, 2, 5, 2)];

	// Layer z3: even smaller
	const z3 = [...points(6, 7, 3, 4, 3)];

	// Layer z4: peak (single tile centered on z3)
	const z4 = [{ x: 6.5, y: 3.5, z: 4 }];
...
const createHeartLayout = () => {
	...
	const z1 = [...points(3, 8, 2, 7, 1)];

	// Layer z2: 4×4 grid (narrower inner)
	const z2 = [...points(4, 7, 3, 6, 2)];

	// Layer z3: 2×2 grid (even narrower)
	const z3 = [...points(4, 7, 3, 5, 3)];
...
export const createBoard = ({ seed, layout = "ClassicTurtle" } = {}) => {
	...
	const randomSeed = Number.isInteger(seed)
		? seed
	...
	if (layoutCells.length !== faces.length) {
		throw new Error(
			`Invalid layout size for ${layout}: expected ${faces.length}, got ${layoutCells.length}`,
		);
	}
...
const isFractionalCoordinate = (value) =>
...
const isPartiallyBlockedByTop = (board, tileId) => {
	...
	return board.tiles.some((potentialCoverer) => {
		...
		const covererRange = {
			xMin: potentialCoverer.x,
			xMax: potentialCoverer.x + 1,
			yMin: potentialCoverer.y,
			yMax: potentialCoverer.y + 1,
		};

		// Check if ranges overlap in both x and y
		const xOverlap =
			tileRange.xMin < covererRange.xMax && tileRange.xMax > covererRange.xMin;
		const yOverlap =
			tileRange.yMin < covererRange.yMax && tileRange.yMax > covererRange.yMin;

		if (!xOverlap || !yOverlap) return false;

		// Check if it's partial coverage (not exact alignment)
		const xExact =
			tileRange.xMin === covererRange.xMin &&
			tileRange.xMax === covererRange.xMax;
		const yExact =
			tileRange.yMin === covererRange.yMin &&
			tileRange.yMax === covererRange.yMax;
...
const isPartiallyBlockedOnSide = (board, tileId) => {
	...
	return board.tiles.some((potentialBlocker) => {
		if (
			potentialBlocker.removed ||
			potentialBlocker.z !== tile.z ||
			potentialBlocker.id === tile.id
		) {
			return false;
		}
		if (
			!hasFractionalPosition(tile) &&
			!hasFractionalPosition(potentialBlocker)
		)
			return false;

		const blockerRange = {
			xMin: potentialBlocker.x,
			xMax: potentialBlocker.x + 1,
			yMin: potentialBlocker.y,
			yMax: potentialBlocker.y + 1,
		};
		...
		const touchesOnLeft =
			Math.abs(blockerRange.xMax - tileRange.xMin) <= EPSILON;
		const touchesOnRight =
			Math.abs(tileRange.xMax - blockerRange.xMin) <= EPSILON;
		...
		const blockerDistanceToCenter = Math.abs(
			potentialBlocker.x + 0.5 - centerX,
		);
