...
describe('hmi persistence and highscore wiring', () => {
  it('exports settings persistence functions', () => {
    ...
    expect(content).toContain('export { saveSettingsToStorage, restoreSettingsFromStorage }');
  });

  it('uses dedicated settings storage key', () => {
    const content = getHmiContent();
    expect(content).toContain("const SETTINGS_STORAGE_KEY = 'mahjong_user_settings'");
    expect(content).toContain('localStorage.getItem(SETTINGS_STORAGE_KEY)');
    expect(content).toContain('localStorage.setItem(SETTINGS_STORAGE_KEY');
  });

  it('contains highscore storage and update functions', () => {
    const content = getHmiContent();
    expect(content).toContain("const HIGHSCORE_STORAGE_KEY = 'mahjong_highscores'");
  ...
  it('asks for confirmation before each reset action', () => {
    ...
    expect(content).toContain("window.confirm('Are you sure?')");
  });
