...
describe("controller worker message handling", () => {
	...
	it("restarts with selected layout", async () => {
		...
		onMessage({
			data: {
				request: "restart",
				settings: { ...defaultSettings, layout: "Square" },
			},
		});
	...
	it("applies valid move action and increments move count", async () => {
		...
		const action = posted.find((m) => m.request === "human_to_move")
			.selectableActions[0];
		posted.length = 0;
	...
	it("rejects invalid showfreetiles/showhints/selectionmode/tiletheme payload fields", async () => {
		...
		expect(posted.find((m) => m.request === "redraw").board.layout).toBe(
			"Square",
		);
		...
		onMessage({
			data: {
				request: "restart",
				settings: { ...defaultSettings, layout: "Heart", showhints: "Maybe" },
			},
		});
		expect(posted.find((m) => m.request === "redraw").board.layout).toBe(
			"Square",
		...
		onMessage({
			data: {
				request: "restart",
				settings: { ...defaultSettings, layout: "Heart", tiletheme: "Sepia" },
			},
		});
		expect(posted.find((m) => m.request === "redraw").board.layout).toBe(
			"Square",
