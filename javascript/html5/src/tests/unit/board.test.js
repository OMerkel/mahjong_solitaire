...
describe("matching and free-tile logic", () => {
	it("matches same groups and rejects non-matches", () => {
		...
		expect(
			canMatch({ id: 1, group: "flower" }, { id: 2, group: "flower" }),
		).toBe(true);
		expect(canMatch({ id: 1, group: "B1" }, { id: 2, group: "C1" })).toBe(
			false,
		);
		expect(canMatch(null, { id: 2, group: "C1" })).toBe(false);
		expect(canMatch({ id: 1, group: "B1" }, { id: 1, group: "B1" })).toBe(
			false,
		);
	});
	...
	it("detects free tiles based on side/top blocking", () => {
		...
		const topBlocked = {
			...
			tiles: [
				...board.tiles,
				{ id: 6, x: 0, y: 0, z: 1, face: "WN", group: "WN", removed: false },
			],
		};
